...
export default class Dataset {
  ...
  public toCsvDataColumns(): CsvDataColumn[] {
    ...
    const emptyData = new Array(this.numObjects).fill(NaN);
    ...
    columns.push({
      name: CSV_COL_CENTROID_X,
      data: this.centroids ? this.centroids.filter((_, index) => index % 3 === 0) : emptyData,
    });
    columns.push({
      name: CSV_COL_CENTROID_Y,
      data: this.centroids ? this.centroids.filter((_, index) => index % 3 === 1) : emptyData,
    });
    columns.push({
      name: CSV_COL_CENTROID_Z,
      data: this.centroids ? this.centroids.filter((_, index) => index % 3 === 2) : emptyData,
    });

    // Write features, excluding auto-generated features (time, track, centroid).
    // TODO: Centroid feature may differ from original data in terms of scaling. Is it better to
    // return the initial centroid data, or the in-viewer generated data?
    const autoGeneratedFeatures = new Set([
      TIME_FEATURE_KEY,
      TRACK_FEATURE_KEY,

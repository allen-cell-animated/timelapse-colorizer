...
import DataCache from "@/colorizer/DataCache";
...
describe("DataCache", () => {
  ...
  describe("caching", () => {
    it("can insert and retrieve values up to capacity", () => {
      const cache = new DataCache<string, DisposableString>(3);
    ...
    it("calls dispose() on entries when they are evicted", () => {
      const cache = new DataCache<string, DisposableString>(1);
    ...
    it("allows values that are non-disposable", () => {
      const cache = new DataCache<string, string[]>(2);
      cache.insert("1", ["A"]);
      cache.insert("2", ["B"]);
      expect(cache.size).toBe(2);
  ...
  describe("reserved keys", () => {
    ...
    it("allows unreserved keys to be evicted", () => {
      const cache = new DataCache<string, DisposableString>(2);
      cache.insert("1", new DisposableString("A"));
      cache.insert("2", new DisposableString("B"));
